                ...
                <v-list v-if="exams.length > 0" >
                    ...
                    <template v-for="(exam, index) in exams" >
                        <v-list-item
                            ...
                            :key="exam.id"
                        >
                            <v-list-item-avatar :color="exam.final ? 'secondary': 'primary'" tile class="rounded-lg" dark>
                            ...
                            <v-list-item-action class="d-none d-md-block text-center">
                                <v-chip 
                                ...
                                <v-icon v-if="exam.feedback" class="mx-1" color="success">
            ...
            <v-card class="rounded-0" v-if="selected !== null" color="primary">
                ...
                <v-card-text class="pt-0 px-0 px-md-6">
                    <v-container style="min-height: 90vh" class="d-flex">
                        <div class="ma-auto" :class="{ 'card-questions': exam_active }">
                            <!-- ESTATUS DEL EXAMEN -->
                            <v-card v-if="!exam_active && !selected.completed" class="pa-5 text-center">
                                <v-card-text class="pa-0">
                                    <div class="my-8">
                                        <p class="text-h5 font-weight-bold">{{selected.name}}</p>
                                    </div>
                                    <v-toolbar flat height="100">
                                        <v-row >
                                            <v-col class="py-0 px-3">
                                                <v-icon class="ma-1">mdi-clock-outline</v-icon>
                                                <p class="ma-1 font-weight-bold">{{ selected.time_format }}</p>
                                                <p class="ma-1 text-caption">Tiempo</p>
                                            </v-col>
                                            <v-divider
                                                class="mx-2"
                                                inset
                                            ...
                                            ></v-divider>
                                            <v-col class="py-0 px-3">
                                                <v-icon class="ma-1">mdi-progress-check</v-icon>
                                                <p class="ma-1 font-weight-bold">{{ valueExam }} puntos</p>
                                                <p class="ma-1 text-caption">Puntos</p>
                                            </v-col>
                                            <v-divider
                                                class="mx-2"
                                                inset
                                            ...
                                            ></v-divider>
                                            <v-col class="py-0 px-3">
                                                <v-icon class="ma-1">mdi-help-circle-outline</v-icon>
                                                <p class="ma-1 font-weight-bold">{{selected.questions.length}}</p>
                                                <p class="ma-1 text-caption">Preguntas</p>
                                            </v-col>
                                        </v-row>
                                    </v-toolbar>
                                    <v-btn class="mt-6" color="secondary" v-if="!exam_active" @click="initExam" :disabled="ended && !selected.completed" block>
                                        Iniciar examen
                                    </v-btn>
                                    <v-btn @click="closeDialog" block class="mt-2">
                                        Cerrar
                                    </v-btn>
                                </v-card-text>
                            </v-card>
                            <div v-else-if="!exam_active && selected.completed">
                                <div class="text-center mb-16 pb-6 white--text">
                                    ...
                                    <p class="text-h2 ma-0 ">{{ (selected.grade * 100).toFixed(2) }}</p>
                                </div>
                                <v-card>
                                    <v-card-text>
                                        <v-row class="my-3 mx-0 mx-sm-4">
                                                ...
                                                <v-list-item class="no-border">
                                                    ...
                                                    <v-list-item-content>
                                                        <v-list-item-title class="success--text font-weight-bold">
                                                            {{selected.questions.length * selected.grade}}
                                                        </v-list-item-title>
                                                        <v-list-item-subtitle>
                                                            Preguntas correctas
                                                        </v-list-item-subtitle>
                                            ...
                                            <v-col class="pa-0">
                                                <v-list-item class="no-border">
                                                    <v-list-item-avatar>
                                                        <v-icon color="accent">mdi-close-circle</v-icon> 
                                                    </v-list-item-avatar>
                                                    <v-list-item-content>
                                                        <v-list-item-title  class="accent--text font-weight-bold">
                                                            {{parseInt(selected.questions.length - (selected.questions.length * selected.grade)) }}
                                                        </v-list-item-title>
                                                        <v-list-item-subtitle>
                                                            Preguntas correctas
                                                        </v-list-item-subtitle>
                                        ...
                                        <v-btn class="mt-6" block color="secondary" v-if="!exam_active" @click="initExam" :disabled="ended && selected.completed" >
                                            Revisar examen
                                        </v-btn>
                                        <v-btn @click="closeDialog" block class="mt-2">
                                            Cerrar
                                ...
                                <v-card outlined class="text-left mt-6" v-if="selected.feedback">
                                    <v-card-text>
                            ...
                            <div v-else class="text-center pa-3 px-md-16">
                                <v-toolbar flat color="primary" dark>
                                    <p class="text-h5 ma-0 ">Pregunta {{ exam.index }}</p>
                                    <v-spacer />
                                    <v-btn outlined @click="exam.resume = true" small v-if="!selected.completed">
                                        <v-icon>
                                            mdi-list-status
                                        </v-icon>
                                    </v-btn>
                                ...
                                <div class="mx-3" v-if="!selected.completed">
                                    <v-progress-linear
                                        color="secondary"
                                        :value="this.exam.answers.filter( a => a !== null).length / this.exam.answers.length * 100"
                                ...
                                <div v-if="exam.questions.length > 0">
                                    <v-card class="pa-4 text-left">
                                        <v-card-text>
                                            ...
                                            <v-list nav :disabled="selected.completed">
                                                <v-list-item-group :color="!selected.completed ? 'secondary' : isCorrect() " v-model="exam.actual_answer" @change="setAnswer">
                                                    <v-list-item 
                                                        v-for="(answer, index) in exam.questions[exam.index - 1].answers" 
                                                        :key="index" 
                                                        class="rounded-lg"
                                                        :class="{ 'correct-answ': index === exam.questions[exam.index - 1].correct && selected.completed }"
                                                    >
                                                        <v-list-item-content>
                                                            <v-list-item-title> 
                                                                <p class="ma-0 answer"> {{answer}} </p> 
                                                            </v-list-item-title>
                                                        </v-list-item-content>
                                                    </v-list-item>
                                                </v-list-item-group>
                                    ...
                                    <v-row>
                                        <v-col>
                                            <v-btn class="mt-3 px-6 mt-6 secondary--text" @click="changeQuestion(exam.index - 1)" block :disabled="exam.index === 1">
                                                Anterior
                                            </v-btn>
                                        </v-col>
                                        <v-col>
                                            <v-btn class="mt-3 px-6 mt-6" @click="changeQuestion(exam.index + 1)" block color="secondary" :disabled="exam.index === exam.questions.length">
                                                Siguiente
                                            </v-btn>
                                        </v-col>
                                    </v-row>
                                    <v-btn class="mt-3 px-6 mt-6" block v-if="!exam.answers.includes(null)" @click="finishExam" color="success">
                                        Finalizar
                                    </v-btn>
                                    <v-btn class="mt-3 px-6 mt-6" block v-else-if="selected.completed" @click="finishExam">
                                        Cerrar
                                    </v-btn>
                                </div>
                            </div>
        ...
        <v-dialog v-model="exam.resume">
            <v-card v-if="exam.resume">
                <v-toolbar flat dense>
                    <p class="my-0 ml-3 font-weight-bold">Preguntas contestadas</p>
                    <v-spacer></v-spacer>
                ...
                <v-card-text>
                    <v-row>
                        <v-col v-for="(question, index) in this.exam.answers" :key="index" cols="12" sm="3">
                            <v-list-item @click="changeQuestion(index + 1)" class="rounded-lg">
                                <v-list-item-content>
                                    <v-list-item-title>
                                        Pregunta {{ index + 1}}
                                    </v-list-item-title>
...
export default {
    ...
    methods: {
        ...
        setAnswer(){
            this.exam.answers[this.exam.index - 1] = 
                isNaN(this.exam.actual_answer)
                    ? null
