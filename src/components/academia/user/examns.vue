                    ...
                    <v-container style="min-height: 90vh" class="d-flex">
                        <div class="ma-auto text-center" :class="{ 'card-questions': exam_active }">
                            ...
                            <v-card v-else class="text-center pa-3">
                                ...
                                <v-card-text class="pa-5 text-left" v-if="exam.questions.length > 0">
                                    <p class="subtitle-1 font-weight-bold">Pregunta {{ exam.index + 1 }}</p>
                                    <p class="text-body-1">{{ exam.questions[exam.index].question }}</p>
                                    <v-list nav>
                                        <v-list-item-group color="primary" v-model="exam.actual_answer">
                                            <v-list-item v-for="(answer, index) in exam.questions[exam.index].answers" :key="index">
                                                <v-list-item-content>
                                    ...
                                    <v-progress-linear rounded :value="exam.index/exam.questions.length * 100" class="mt-3"></v-progress-linear>
                                </v-card-text>
                            </v-card>
                            <v-btn class="mt-3 px-6" v-if="exam_active" @click="nextQuestion" :disabled="exam.actual_answer === undefined">
                                <span v-if="exam.index !==  exam.questions.length - 1">Siguiente</span>
                                <span v-else>Finalizar</span>
                            </v-btn>
                            <v-btn class="mt-6 px-6" v-else-if="!selected.completed"  @click="initExam">
                                Iniciar examen
                            </v-btn>
                        </div>
                    </v-container>
...
export default {
    ...
    methods: {
        ...
        closeDialog(){
            if(this.exam_active) this.finishExam()
            this.selected = null
            this.dialog = false
        },
        initExam(){
        ...
        finishExam(){
            ...
            this.setCompleted({ id: this.selected.id, criteria: EXAMS, options: { grade: result } })
            this.exam.questions = []
            this.exam_active = false
            this.closeDialog()
        },
        nextQuestion(){
            if(this.exam.index + 1 > this.exam.questions.length - 1){
                this.exam.answers.push(this.exam.actual_answer)
                this.finishExam()
            }else{
                this.exam.answers.push(this.exam.actual_answer)
