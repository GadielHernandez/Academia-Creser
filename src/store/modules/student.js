...
const actions = {
    async fetchCourse({ commit, state }) {
        ...
        const user = await db
            .doc(`users/${user_id}/courses/${state.course_id}`)
            .get()
        ...
        const group = await db
            .doc(`courses/${state.course_id}/groups/${user_data.group}`)
            .get()
    ...
    fetchLessons({ commit, state }) {
        return new Promise((resolve, reject) => {
            const now = timeServer().toMillis()
            db.collection(`courses/${state.course_id}/lessons`)
                .where('available_after', '<', now - state.group.starts)
                .get()
                .then((lessons) => {
                    commit(
                        'UPDATE_LESSONS',
                        lessons.docs.map((l) => ({ id: l.id, ...l.data() }))
                    )
                    if (state.initial_user_data.lessons_seen)
                        commit(
                            'SET_LESSONS_SEEN',
                            state.initial_user_data.lessons_seen
                        )
                    resolve()
                })
                .catch((e) => reject(e))
    ...
    fetchTasks({ commit, state }) {
        return new Promise((resolve, reject) => {
            const now = timeServer().toMillis()
            const { tasksExpirationTime } = state.info

            db.collection(`courses/${state.course_id}/tasks`)
                .where('available_after', '<', now - state.group.starts)
                .get()
                .then((tasks) => {
                    ...
                    commit('UPDATE_TASKS', list_tasks)
    ...
    uploadSeenLesson({ state, commit }, lesson) {
        return new Promise((resolve, reject) => {
            ...
            db.doc(`users/${auth.currentUser.uid}/courses/${state.course_id}`)
                .update({ lessons_seen: addLesson })
                .then(() => {
    ...
    fetchExams({ commit, state }) {
        return new Promise((resolve, reject) => {
            const now = timeServer().toMillis()
            db.collection(`courses/${state.course_id}/exams`)
                .where('available_after', '<', now - state.group.starts)
                .get()
                .then((exams) => {
                    commit(
                        ...
                        exams.docs.map((e) => {
                            ...
                            const data = e.data()
                            let hour = parseInt(
                                data.limit_time / (1000 * 60 * 60)
                            )
                            let minutes = parseInt(
                                (data.limit_time - hour * (1000 * 60 * 60)) /
                                    (1000 * 60)
                            )
                            let seconds =
                                (data.limit_time -
                            ...
                            return {
                                id,
                                ...data,
                                time_format: `${hour}:${minutes}:${seconds}`,
                                course_start: state.group.starts,
                            }
                        })
                    )
                    ...
                    if (state.initial_user_data.feedback_exams) {
                        state.initial_user_data.feedback_exams.forEach(
                            (feedback) => {
...
const mutations = {
    ...
    UPDATE_CRITERIA_COMPLETE(state, payload) {
        ...
        if (progress[payload.criteria] == undefined)
            ...
            progress[payload.criteria].push({
                id: payload.id,
                ...payload.value,
            })
